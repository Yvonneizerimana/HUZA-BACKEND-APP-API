...
import { BadRequestError } from "/Users/USER/Desktop/bsa/errors/BadRequestError.js";
...
const skilledController = {
  signup: async (req, res, next) => {
    ...
    //   const hashedPassword = await bcrypt.hash(req.body.password, 10);
      ...
      // Create a new user
      const newUser = await skilledModel.create({
  ...
  allUser: async (req, res) => {
    try {
      const allUser = await skilledModel.find().select(-password);
      res.status(200).json({
        message: "All user",
        allUser: allUser,
      });
  ...
  updating: async (req, res) => {
    try {
      ...
      const user = await skilledModel.findById(req.params.id);
      if (!user) {
        return res.status(404).json({ message: "User not found" });
      }
      const updatedUser = await skilledModel.findByIdAndUpdate(
        req.params.id,
        req.body,
        { new: true }
      );
  ...
  Deleting: async (req, res) => {
    try {
      ...
      await skilledModel.findByIdAndDelete(req.params.id);
      res.status(200).json({
  ...
  listByid: async (req, res) => {
    try {
      const user = await skilledModel.findById(req.params.id).select(-password);
      if (!user) {
        return res.status(404).json({ message: "User not found" });
      }
      res.status(200).json({
  ...
  login: async (req, res) => {
    try {
      ...
      const user = await skilledModel.findOne({ email });
  ...
  ResetPassword: async (req, res, next) => {
    try {
      ...
      // Find user
      const foundUser = await skilledModel.findById(req.body.id);
      if (!foundUser) {
...
export default skilledController;
